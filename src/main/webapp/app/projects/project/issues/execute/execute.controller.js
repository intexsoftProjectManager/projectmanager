...
app.controller('IssuesExecuteController', IssuesExecuteController);

function IssuesExecuteController($scope, $rootScope, Builds, Environments, ExecutedChecklists, Iterations, ngDialog, poollingFactory, priorities, $state) {
  ...
  $scope.loadMoreSteps = function() {
      var renderedStepsCount = $scope.renderedSteps.length;
      var allStepsCount = $scope.checklist.steps.length;
      for (var i = renderedStepsCount; i < renderedStepsCount + 8; i++) {
        if ($scope.renderedSteps.length + 1 > allStepsCount) {
          return;
        }
        $scope.renderedSteps.push($scope.checklist.steps[i]);
      }
  };
...
IssuesExecuteController.resolve = {
