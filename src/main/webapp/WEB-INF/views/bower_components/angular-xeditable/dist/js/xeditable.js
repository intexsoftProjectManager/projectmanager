...
angular.module('xeditable').directive('editableTextarea', ['editableDirectiveFactory',
  function(editableDirectiveFactory) {
    return editableDirectiveFactory({
      ...
      inputTpl: '<text-angular ng-model="$data" ta-toolbar="[[\'bold\',\'italics\',\'underline\'],[\'ul\',\'ol\'],[\'undo\',\'redo\',\'clear\'] ]"></text-angular>',
...
angular.module('xeditable').factory('editableController',
  ...
  EditableController.$inject = ['$scope', '$attrs', '$element', '$parse', 'editableThemes', 'editableOptions', '$rootScope', '$compile', '$q', 'mbScrollbar'];
  function EditableController($scope, $attrs, $element, $parse, editableThemes, editableOptions, $rootScope, $compile, $q, mbScrollbar) {
    var valueGetter;
    ...
    self.activate = function() {
      setTimeout(function() {
        ...
        if (editableOptions.activate === 'focus' && el.focus) {
          //VCH: focus for new angular-textarea
          if (el.tagName === "TEXT-ANGULAR") {
            var editableDiv = $(el).find("div[contenteditable='true']").get(0);
            ...
            function scrollToView(element){
              var container = element.parents('.scroll-wrapper');

              var scrollingarea = element.parents('.scroll-wrapper').children();

              var offset =  element.parents('.row')[0].offsetTop + 60;
              ...
              var visible_area_start = container.scrollTop() - container.height();
              var visible_area_end = visible_area_start + container.height();

              if(offset < visible_area_start || offset > visible_area_end){
                // Not in view so scroll to it
                container.animate({scrollTop: offset - container.height()/3}, 500);
                return false;
              }
              return true;
            }
            scrollToView(self.inputEl);
          } else {
            el.focus();
          }
        }
      ...
      compile: function() {
        return {
          ...
          post: function(scope, elem, attrs, ctrl) {
            ...
            elem.find('div[contenteditable="true"]').mousedown(function(e) {
              // ignore right/middle button click
              if (e.which !== 1) {
                return;
              }
