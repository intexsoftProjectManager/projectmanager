...
function Calendar(element, options, eventSources) {
	...
	t.option = option;
	t.trigger = trigger;
    t.resourceFilter = options.resourceFilter;
    t.getFilter = getFilter;
    t.windowResize = windowResize;
    t.hasVerticalScroll = hasVerticalScroll;
    t.holidaysSources = t.holidays ? renderHolidays() : null;
	...
	function initialRender() {
        ...
        if (t.resourceFilter) {
            ...
            t.currentFilter = element.find(".fc-resource-filter").val();
        }
    ...
    function renderResourceFilter(){
        element.find(".fc-resource-filter").select2({
            minimumInputLength: 0,
            ajax: {
                url: t.resourceFilter,
                dataType: 'json',
                type: "GET",
                quietMillis: 50,
                data: function (term) {
                    return { term: term, end: currentView.end, start: currentView.start };
                },
        ...
        //Support for resource filter
        var resourceFilter = element.find('.fc-resource-filter');
        resourceFilter.select2('data', {id: "-1", text: 'All'});
        resourceFilter.on("change", function(e) {
          changeView(currentView.name, true);
          /*var selectedID = resourceFilter.select2('data').id;
          var tableBody = element.find('.floatThead-wrapper .fc-view table tbody');
          element.find('.fc-event').hide();
          if (selectedID !== "-1") {
            tableBody.children('tr:not(.fc-resourcerow-' + selectedID + ')').hide();
            tableBody.children('tr:not(.fc-resource-group-' + selectedID + ')').hide();
            tableBody.children('tr.fc-resourcerow-' + selectedID).show();
            tableBody.children('tr.fc-resource-group-' + selectedID).show();
          } else {
    ...
    function renderHolidays(){
        $.ajax({
            ...
            success: function (data) {
                t.holidaysSources = data;
            },
            complete: function () {
            },
            error: function () {
                t.holidaysSources = null;
            },
            url: t.holidays,
            cache: false
        });
        return t.holidaysSources;
    }
	...
	function changeView(newViewName, force) {
		if (!currentView || newViewName != currentView.name || force) {
	...
	function setSize(dateChanged) { // todo: dateChanged?
		ignoreWindowResize++;
        //TODO: VCH Nothing happens if we no do this?
		//currentView.setHeight(suggestedViewHeight, dateChanged);
		if (absoluteViewElement) {
			absoluteViewElement.css('position', 'relative');
			absoluteViewElement = null;
		}
		currentView.setWidth(content.get(0).clientWidth, dateChanged);
		ignoreWindowResize--;
	}
	...
	function updateEvents(forceRender) {
		if (!options.lazyFetching || isFetchNeeded(currentView.visStart, currentView.visEnd)) {
			refetchEvents();
			if (options['refetchResources']) refetchResources(currentView); // refetch resources every time new events are loaded
		}
		else if (forceRender) {
			rerenderEvents();
		}
	...
	function refetchEvents() {
        ...
        if(hasVerticalScroll()) windowResize();
    }

	function refetchResources(currentView) {
    ...
    function getFilter() {
        return $($(".fc-resource-filter")[1]).val();
    }
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
                    // add native filter support
                    // by Vadim Chadyuk
                    if (buttonName == 'resourceFilter') {
                        if (t.resourceFilter) {
                            e.append('<input class="fc-resource-filter" style="height:35px; width:280px">');
                        }
                    } else
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							var button = $(
								"<span class='"+(icon ? "" : "fc-button")+" fc-button-" + buttonName + "'>" +
									(icon ?
...
function EventManager(options, _sources) {
	...
	function _fetchEventSource(source, callback) {
        ...
        data['filter'] = $($(".fc-resource-filter:visible")[1]).val();
...
function ResourceManager(options, view) {
    var t = this;
    ...
    function _fetchResourceSource(source, currentView) {
        ...
        if (resources) {
            ...
            if (url) {
                var data={};
                if (currentView) {
                    data["start"] = currentView.visStart;
                    data["end"] = addDays(currentView.visEnd,-1);
                    data['filter'] = $($('.fc-resource-filter:visible')[1]).val();
                    addDays(currentView.visEnd,1);
                }
                $.ajax($.extend({}, source, {
                    data: data,
                    dataType: 'json',
			...
			if (eventElement = seg.element) {
				...
				if (seg.contentTop !== undefined && height - seg.contentTop < 10) {
					...
					eventElement.find('div.fc-event-time')
						.text(formatDate(event.start, opt('timeFormat')) + ' - ' + htmlEscape(event.title));
...
function ResourceMonthView(element, calendar) {
	...
	var renderBasic = t.renderBasic;
	var formatDates = calendar.formatDates;
    var getResources = calendar.fetchResources;
	...
	function render(date, delta) {
        ...
        t.getResources = getResources(false,t);
		renderBasic(t.getResources.length, t.getResources.length, cols, false);
	}
...
function ResourceWeekView(element, calendar) {
	...
	function render(date, delta) {
		if (delta === 100 || delta === -100) {
            ...
            //if we hav not delta, than start from current day week @Vadim Chadyuk
            var currentDate = new Date();
            var start = addDays(cloneDate(currentDate), -((currentDate.getDay() - opt('firstDay') + 7) % 7));
			var end = addDays(cloneDate(start), 7);
		}
        ...
        t.getResources = getResources(false,t);
		renderBasic(t.getResources.length, t.getResources.length, weekends ? 7 : 5, false);
	}
}
...
function ResourceView(element, calendar, viewName) {
	...
	// imports
	var getView = View.call(t, element, calendar, viewName);

    //t.getResources = calendar.fetchResources;
    OverlayManager.call(t);
	SelectionManager.call(t);
	ResourceEventRenderer.call(t, calendar);
	var opt = t.opt;
	var trigger = t.trigger;
	var clearEvents = t.clearEvents;
	var renderOverlay = t.renderOverlay;
	...
	function renderBasic(maxr, r, c, showNumbers) {
		...
		updateOptions();
		var firstTime = !body;
		if (firstTime || viewName == 'resourceMonth' || viewName == 'resourceWeek') {
    ...
    var $table = $(element).find('table:not(.floatThead-table)');
    var viewWidth = element[0].clientWidth - 15;
    var colWidth = Math.floor((viewWidth - 200) / colCnt);
    //colWidth = 200;
    $table.floatThead('destroy');
		s =
			"<table class='fc-border-separate' style='width:100%' cellspacing='0'>" +
			"<thead>" +
			"<tr class='fc-first fc-last'><th class='fc-resourceName fc-widget-header' style='white-space:nowrap;min-width:200px;width:200px;height:34px'></th>";
		for (i=0; i<colCnt; i++) {
			s +=
				"<th class='fc- " + headerClass + "' style='width:99%;width:"+colWidth+"px;height:34px'/>";
		}
		s +=
			"</tr>" +
			"</thead>" +
      ...
      var resourceGroup = resources[i]['group'];
      var prefix = "";
      //* add odd support
      //* by Vadim Chadyuk
      if (id === 'resource-group-title' && (row + i) % 2 != 0 )
          row++;
      if ((row + i) % 2 === 0)
          prefix  = " odd";
      else
          prefix  = " even";
			    s +=
				    "<tr class='fc-resourcerow-" + id + prefix + " fc-resource-group-" + resourceGroup + "'><td class='fc-resourceName'  style='max-width:200px; display:inline-block;'><div class='ellipsis' data-toggle='tooltip' title='"+htmlEscape(resourceName)+"' data-placement='right'>" + htmlEscape(resourceName) + "</div></td>";
			for (j=0; j<colCnt; j++) {
				s +=
    ...
    table = $(element);
    $table = table.find('table:not(.floatThead-table)');
    $table.floatThead({
      scrollContainer: function ($table) {
        return $table.closest('.fc-view');
      }
    });
	}
	...
	function updateCells(firstTime) {
		...
		headCells.each(function(i, _cell) {
			cell = $(_cell);
			date = indexDate(i);
            //delete colFormat number in header @VadimChadyuk
			cell.html(formatDate(date, colFormat));
            //TODO: add new implementation for holidays
            /*//add support for holidays in header @Vadim Chadyuk
            if(calendar.holidays && calendar.holidaysSources) {
                var holidayDate;
                var day;
                var month;
                for (var j = 0; j < calendar.holidaysSources.length; j++) {
                    holidayDate = new Date(calendar.holidaysSources[j]);
                    day = holidayDate.getDate();
                    month = holidayDate.getMonth();
                    if ((new Date(+date)).getMonth() === month && (new Date(+date)).getDate() === day) {
		...
		indexCorrecter=0;
        //TODO: VCh improve performance
		bodyCells.each(function(i, _cell) {
			cell = $(_cell);
			date = indexDate(i);
            var isResourceGroup = cell.hasClass( "fc-resourceresource-group-title");
            //TODO: add new implementation for holidays
            /*//add support for holidays @Vadim Chadyuk
            if(calendar.holidays && calendar.holidaysSources && !isResourceGroup) {
                var holidayDate;
                var day;
                var month;
                for (var j = 0; j < calendar.holidaysSources.length; j++) {
			...
			if ((date.getDay() == 0 || date.getDay() == 6) && !isResourceGroup) cell.addClass('fc-weekend-column');
		});
        //TODO: Why use show? Very slow performance with it
	...
	function setWidth(width) {
		...
		var resourceWidth = $('th.fc-resourceName').get(0).clientWidth;
		colContentPositions.clear();
		colWidth = Math.floor((viewWidth - resourceWidth) / colCnt);
	...
	function dayBind(days) {
        days.on('dblclick',function(e){e.preventDefault();e.stopPropagation();}).on('click', dayClick).on('mousedown',daySelectionMousedown);
    }
	...
	function daySelectionMousedown(ev) {
                ...
                var fisrtOrigCell;
                var firstOrigRow;
                // * Some hack for selection
                // * if resource not in selected date range, we can't select cells
                // * by Vadim Chadyuk
                if (ev.which === 1 && opt('selectable')) { // which==1 means left mouse button
                    ...
                    hoverListener.start(function (cell, origCell) { // TODO: maybe put cellDate/cellIsAllDay info in cell
                        if (cell && firstOrigRow != cell.row) {
                            firstOrigRow = cell.row;
                            fisrtOrigCell = null;
                        }
                        clearSelection();
                        if (cell) {
                            var itemDate = cellDate(cell).valueOf();
                            var startDate = resources[cell.row].startDate ? (new Date(resources[cell.row].startDate)).valueOf() : 0;
                            var endDate = resources[cell.row].endDate ? (new Date(resources[cell.row].endDate)).valueOf() : 0;
                            ...
                            if (itemDate >= startDate && itemDate <= endDate) {
                                ...
                                fisrtOrigCell = origCell;
                            }
                        }
                        if (cell && cellIsAllDay(cell) && resourceRO !== true) {
                            dates = [ cellDate(fisrtOrigCell), cellDate(cell) ].sort(cmp);
...
function ResourceEventRenderer(calendar) {
	var t = this;
	...
	// imports
	DayEventRenderer.call(t, calendar);
	var opt = t.opt;
	var trigger = t.trigger;
	//var setOverflowHidden = t.setOverflowHidden;
	var isEventDraggable = t.isEventDraggable;
	...
	function compileSegs(events) {
		var rowCnt = t.getResources.length,
			colCnt = getColCnt(),
...
function View(element, calendar, viewName) {
	...
	*/
	...
	function eventDrop(e, event, dayDelta, minuteDelta, allDay, ev, ui, resource) {
		...
		reportEventChange(eventId);
        //TODO: Implement some more good stuff
        if(!calendar.hasVerticalScroll()) calendar.windowResize();
	}
...
function DayEventRenderer(calendar) {
	var t = this;
	...
	function renderDaySegs(segs, modifiedEventId) {
		...
		for (rowI=0; rowI<rowDivs.length; rowI++) {
			levelI = 0;
			colHeights = [];
			for (j=0; j<colCnt; j++) {
				colHeights[j] = 0;
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
      ...
      for (var count=0;count<event.resource.length;count++) {
        classes.push('fc-resource-' + event.resource[count]);
      }
            ...
            var height = "";
            if(event.id === "iteration-data-range")
                height = "height:2px;";
			html +=
				" class='" + classes.join(' ') + "'" +
				" style='"+height+"position:absolute;z-index:8;left:"+left+"px;" + skinCss + "'" +
				">";
            /*if (seg.isEnd && isEventResizable(event)) {
                html +=
            ...
            html +="<div class='fc-event-inner' title='"+htmlEscape(event.task ? event.task._iteration.name : "")+"'" +
				(skinCss ? " style='" + skinCss + "'" : "") +
				">";
			if (!event.allDay && seg.isStart) {
				html +=
					"<span class='fc-event-time' style='white-space: nowrap'>" +
					htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) +
					"</span>";
			}
			html +=
	...
	function getRowDivs() {
		...
		for (i=0; i<rowCnt; i++) {
            if (allDayRow(i)
                .find('td:not(.fc-resourceName):first div.fc-day-content > div')[0] !== undefined)
    ...
    var divRows = getRowDivs();
        ...
        //hide events that is not visible
        if (!divRows[seg.row].is(':visible'))
          element.hide();
			}
		}
	}
...
function CoordinateGrid(buildFunc) {
    ...
    var origin = originElement.offset();
		return {
			top: rows[row0][0] - origin.top + originElement[0].scrollTop,
	...
	function mouse(ev) {
		...
		if (!newCell != !cell || newCell && (newCell.row != cell.row || newCell.col != cell.col)) {
			if (newCell) {
				...
				change(newCell, firstCell, newCell.row-firstCell.row, newCell.col-firstCell.col, ev.target);
			}else{
				change(newCell, firstCell, ev);
			}
			cell = newCell;
		}
	}
